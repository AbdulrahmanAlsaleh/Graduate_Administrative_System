...
#messages receiving and sending
...
#sending mail to someone with an id
...
@app.route('/approve_advising_hold', methods=['GET', 'POST'])#approving advising hold
...
def GS_queries():
    ...
    if session['user_type'] != "admin":
      if session['user_type'] != "GS":
        return redirect('/')
    cursor = mydb.cursor(dictionary = True)
    if request.method == "POST":#queries for the grad sec
